...
bool OctomapFromGazeboWorld::ServiceCallback(
  ...
  res.origin_latitude = origin_spherical[0];
  res.origin_longitude = origin_spherical[1];
  res.origin_altitude = origin_spherical[2];
  return true;
}
...
void OctomapFromGazeboWorld::FloodFill(
  ...
  to_check.push(ignition::math::Vector3d (seed_point[0], seed_point[1], seed_point[2]));

  while (to_check.size() > 0) {
    ignition::math::Vector3d p = to_check.top();

    if ((p[0] > bounding_box_origin[0] - bounding_box_lengths[0] / 2) &&
        (p[0] < bounding_box_origin[0] + bounding_box_lengths[0] / 2) &&
        (p[1] > bounding_box_origin[1] - bounding_box_lengths[1] / 2) &&
        (p[1] < bounding_box_origin[1] + bounding_box_lengths[1] / 2) &&
        (p[2] > bounding_box_origin[2] - bounding_box_lengths[2] / 2) &&
        (p[2] < bounding_box_origin[2] + bounding_box_lengths[2] / 2) &&
        (!octomap_->search(p[0], p[1], p[2]))) {
      octomap_->setNodeValue(p[0], p[1], p[2], 0);
      to_check.pop();
      to_check.push(ignition::math::Vector3d (p[0] + leaf_size, p[1], p[2]));
      to_check.push(ignition::math::Vector3d (p[0] - leaf_size, p[1], p[2]));
      to_check.push(ignition::math::Vector3d (p[0], p[1] + leaf_size, p[2]));
      to_check.push(ignition::math::Vector3d (p[0], p[1] - leaf_size, p[2]));
      to_check.push(ignition::math::Vector3d (p[0], p[1], p[2] + leaf_size));
      to_check.push(ignition::math::Vector3d (p[0], p[1], p[2] - leaf_size));
